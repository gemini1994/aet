...
double best_fslowdown,cur_fslowdown;
...
int main(int argv, char **argc) {
    ...
    best_fslowdown = predict_fair_weighted_slowdown(CPI,PENALTY);
    cur_fslowdown = best_fslowdown;
    int _count = 0;
    while (T >= T_min) {
        ...
        display();
        double tmp = predict_fair_weighted_slowdown(CPI,PENALTY);
        printf("Fair weighted slowdown: %lf\n",tmp);
        if (tmp < best_fslowdown)
            best_fslowdown = tmp;
        double df = tmp - cur_fslowdown;
