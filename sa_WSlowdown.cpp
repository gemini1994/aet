...
double best_wslowdown,cur_wslowdown;
...
int main(int argv, char **argc) {
    ...
    best_wslowdown = predict_weighted_slowdown(CPI,PENALTY);
    cur_wslowdown = best_wslowdown;
    int _count = 0;
    while (T >= T_min) {
        ...
        display();
        double tmp = predict_weighted_slowdown(CPI,PENALTY);
        printf("Total weighted slowdown: %lf\n",tmp);
        if (tmp < best_wslowdown)
            best_wslowdown = tmp;
        double df = tmp - cur_wslowdown;
